# pages/12_Evaluasi_Skenario_1.py
import streamlit as st
import pandas as pd
import utils

# --- KONFIGURASI HALAMAN ---
st.set_page_config(page_title="Evaluasi Skenario 4", page_icon="🧪", layout="wide")

st.title("🧪 Evaluasi Penuh - Skenario 4")
st.header("Seleksi Fitur & Penghapusan Outlier", divider="blue")
st.markdown("Halaman ini menguji performa Skenario 4, pendekatan paling komprehensif dengan **seleksi fitur** dan **penghapusan outlier**.")
...
if st.button(f"Jalankan Evaluasi Penuh untuk {SCENARIO_NAME}", type="primary"):
    with st.status(f"Mengevaluasi '{SCENARIO_NAME}' di 12 stasiun...", expanded=True) as status:
        result_df, logs = utils.run_evaluation_for_all_stations(
            ...
            use_fs=True,   # <-- Parameter Skenario 4
            use_or=True    # <-- Parameter Skenario 4
        )
